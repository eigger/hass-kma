...
*   **레이더/위성 이미지 (`image.kma_radar_image`, `image.kma_satellite_image`)** ✅ 실제 authKey로 동작 검증 완료(2026-07-01):
    *   레이더 합성영상 PNG(강수 분포도, 범례·시각 포함)와 천리안위성(GK2A) 적외영상 PNG를 최신 스냅샷으로 제공합니다 (허브 단위 엔티티, Zone과 무관).
...
| 기상청 APIhub | `image.kma_radar_image` | 레이더 합성영상 | `typ04/rdr_cmp_file.php` (data=img) ✅검증됨 | 10분 |
| 기상청 APIhub | `image.kma_satellite_image` | 위성(GK2A) 적외영상 | `typ03/nph-gk2a_img` ✅검증됨 | 10분 |
